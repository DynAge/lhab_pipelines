...
def move_excluded_t1w_scans(source_path, mri_qc_path, exclusion_path, exclusion_file):
    """ Function takes exclusion file moves t1w/json and mriqc files to exclusion_path
...
def move_excluded_scans(source_path, exclusion_path, exclusion_file):
    """ Function takes exclusion file moves scans(and addiational files json/bvec...) to exclusion_path
    exclusion file: (excel file with exclue ({nan, 1}) column and excludes exclude==1 scans
    subject_id	session_id	run_id	modality	scan	exclude	info
    lhabXX1	tp1	1	dwi	sub-lhabXX1_ses-tp3_acq-ap_run-1_dwi	1
    ...
    for i in range(len(bad_scans)):
        df_subject = bad_scans.iloc[[i]]
        bad_scan = df_subject.scan.tolist()[0]
        subject = "sub-" + df_subject.subject_id.tolist()[0]
        modality = df_subject.modality.tolist()[0]
        session_id = "ses-" + df_subject.session_id.tolist()[0]
        move_files = {}

        paths = {modality: os.path.join(source_path, subject, session_id, modality),
                 }
        ...
        for kind, p in paths.items():
            search_str = os.path.join(p, bad_scan + "*")
            search_files = sorted(glob(search_str))
            print(search_str, search_files)
            if search_files:
                move_files[kind] = search_files
                removed_scans.append(os.path.join(modality, bad_scan))
            else:
                target_path = os.path.join(exclusion_path, kind)
                search_str = os.path.join(target_path, bad_scan + "*")
                search_files = sorted(glob(search_str))
        ...
        for scan in removed_scans:
            scans_file = os.path.join(source_path, subject, subject + "_scans.tsv")
            scans = pd.read_csv(scans_file, sep="\t")
            removed_line = scans[scans.filename.str.contains(scan)]
            scans = scans[~scans.filename.str.contains(scan)]
            scans.to_csv(scans_file, sep="\t", index=False)
            log = ["removed " + scan + " from " + scans_file + "\n"]
            write_log(log_file, log)
